...
//        logger.error("error：进入error方法");
    ...
    @Test
    public void getUserLimitPageTest(){
        SqlSession sqlSession =MybatisUtils.getSqlSession();
        UserInfoDao mapper = sqlSession.getMapper(UserInfoDao.class);
        Map<String,Object> map = new HashMap<>();
        map.put("startIndex",2);
        map.put("pageSize",1);
        List<Userinfo> users =mapper.getUserLimitPage(map);
        for(Userinfo user:users){
            System.out.println(user);
        }
        sqlSession.close();
    ...
    @Test
    public void getUserAndAccountTest(){
        SqlSession sqlSession = MybatisUtils.getSqlSession();
        UserInfoDao mapper = sqlSession.getMapper(UserInfoDao.class);
        Userinfo user = mapper.getUserAndAccount(5);
            System.out.println(user);
        sqlSession.close();
    }

    @Test
    public void getUserAndAccount2Test(){
        SqlSession sqlSession = MybatisUtils.getSqlSession();
        UserInfoDao mapper = sqlSession.getMapper(UserInfoDao.class);
        Userinfo user = mapper.getUserAndAccount2(3);
        System.out.println(user);
        sqlSession.close();
    }
}
